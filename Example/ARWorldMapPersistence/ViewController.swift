...
class ViewController: UIViewController, ARSCNViewDelegate, ARSessionDelegate, ARWorldMapPersistence {
    // MARK: - IBOutlets
    ...
    @IBAction func saveExperience(_ button: UIButton) {
        worldMapData(from: sceneView) { worldMapData, error in
            do {
                if let error = error {
                    throw error
                }
    ...
    @IBAction func loadExperience(_ button: UIButton) {
        if let worldMapData = mapDataFromFile {
            do {
                if let (worldMap, snapshot) = try worldMap(from: worldMapData) {
                ...
                showAlert(title: "Can't load map", message: error.localizedDescription)
            }
        }
        else {
            showAlert(title: "Can't load map", message: "Data file couldn't be loaded")
        }
    }
